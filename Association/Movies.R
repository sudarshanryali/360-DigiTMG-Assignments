# Prepare rules for the dataset "Movies.csv"
...
# Loading the data
Movies <- read.csv(file.choose(), colClasses = "factor")
...
arules <- apriori(movie, parameter = list(support = 0.002, confidence = 0.75, minlen = 2))
arules
...
write(arules, file = "a_rules_movie.csv", sep = ",")
getwd()
...
arules1 <- apriori(movie, parameter = list(support = 0.001, confidence = 0.95, minlen = 2))
arules1
...
arules2 <- apriori(movie, parameter = list(support = 0.013, confidence = 0.60, minlen = 3))
arules2
