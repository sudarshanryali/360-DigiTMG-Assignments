...
library(caret)
inTraininglocal <- createDataPartition(df$ Diagnosis, p = 0.75, list = F)
training <- df[inTraininglocal, ]
testing <- df[-inTraininglocal, ]

#install.packages("C50")
library(C50)
model <- C5.0(training$ Diagnosis ~ ., data = training[, -5])
plot(model)
...
for(i in 1:11)
{
  ...
  testing <-df[-inTraininglocal, ]
  fittree <- C5.0(training1$ Diagnosis ~ ., data = training1[, -5])
...
model <- C5.0(training$ Diagnosis ~ ., data = training[, -5], trials = 10)
pred <- predict.C5.0(model, testing[, -5])
a <- table(testing$ Diagnosis, pred)
